...
static int window_init( WININFO *info )
{
    ...
    wc.hbrBackground =(HBRUSH)CreateSolidBrush(0x00);
...
int WINAPI WinMain( HINSTANCE instance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
{
  ...
  //if( MessageBox( 0, "fullscreen?", info->wndclass, MB_YESNO|MB_ICONQUESTION)==IDYES ) info->full++;
	...
	int		result;
    char    error[1024];
    oglGetObjectParameteriv( vsId, GL_OBJECT_COMPILE_STATUS_ARB, &result ); oglGetInfoLog( vsId, 1024, NULL, (char *)error ); if( !result ) { MessageBox(0, error, "Vertex Shader Error", MB_OK|MB_ICONERROR); return 1; }
    oglGetObjectParameteriv( fsId, GL_OBJECT_COMPILE_STATUS_ARB, &result ); oglGetInfoLog( fsId, 1024, NULL, (char *)error ); if( !result ) { MessageBox(0, error, "Fragment Shader Error", MB_OK|MB_ICONERROR); return 1; }
    oglGetObjectParameteriv( pid,  GL_OBJECT_LINK_STATUS_ARB,    &result ); oglGetInfoLog( pid,  1024, NULL, (char *)error ); if( !result ) { MessageBox(0, error, "Link Error", MB_OK|MB_ICONERROR); return 1; }
